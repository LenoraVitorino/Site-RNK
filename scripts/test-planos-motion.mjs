...
for(let i=0;i<4;i++){
 ...
 assert.match(layers[i+1].style.clipPath,/50\.000%/);
}
...
// Symbols must be descendants of their corresponding article so the wipe covers them.
...
console.log('PASS: five plans forward/reverse, directional assembly, static outgoing layers, dark overlay, wipe, inactive links, header exit, reduced-motion/mobile reset and desktop restore.');
