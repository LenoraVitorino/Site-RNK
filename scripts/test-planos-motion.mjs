...
const mapa=layers.map(()=>new Element());
...
track.querySelectorAll=k=>({'[data-plano]':layers,'[data-icone]':icons,'[data-nome]':names,'[data-plano-ir]':anchors,'[data-mapa]':mapa}[k]);
...
for(const i of [0,1,2,3,4,3,2,1,0]){scroll(1000+i*900);assert.equal(layers.filter(x=>!x.inert).length,1);assert.equal(layers[i].inert,false);assert.ok(mapa[i].classes.has('mapa__plano--ativo'));assert.ok(Number(names[i].style.opacity)>.99);assert.ok(Number(icons[i].style.opacity)>.99)}
