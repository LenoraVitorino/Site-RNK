 ...
 * - nada acompanha o ponteiro (lá, o desfoque em volta do mouse);
 ...
 * leitura: quanto a máscara escurece o anel atrás do texto.
 */
const q = (x: number, y: number, z: number, rx: number, ry: number, rz: number, s: number, luz: number, onda = 1, leitura = .88): Quadro =>
  ({ x, y, z, rx, ry, rz, s, luz, onda, leitura });

const QUADROS = {
  desktop: {
    // Grande, à direita do título, inclinado e saindo da tela, como a hero de lá.
    hero:        q(3.4, .1, 0, 62, -16, 28, 1.45, 1),
    // Vem para trás do vídeo, quase de frente: o vídeo ganha um halo.
    metodologia: q(-3.2, -.2, -3, 18, 34, -12, 1.15, .62),
    // Deita e recua: um horizonte atrás da grade de cartões.
    pilares:     q(0, -.4, -6, 84, 0, 4, 2, .5, .6),
    ...
    // Fecha calmo, à esquerda do formulário.
    formulario:  q(-3.8, .3, -1.5, 56, 24, -30, 1.2, .6),
  },
  celular: {
    hero:        q(1.2, 1.9, -1, 64, -16, 26, .78, .9),
    metodologia: q(-1, 2.2, -3, 18, 34, -12, .8, .5),
    pilares:     q(0, -2.4, -6, 84, 0, 4, 1.3, .42, .6),
...
uniform float uTempo, uIntensidade, uQuadro;
...
void main() {
  ...
  float k = w * float(${FIOS}) + 2.2 * sin(w * 3. + uTempo * .18);
  ...
  float forca = ruido(vec2(w * 7.5, vUv.x * 9. + uTempo * .05)) * ruido(vec2(w * 2.1 + 3., vUv.x * 3. - uTempo * .03));
...
function criar(ctx: Contexto): Elemento {
  ...
  const u = {
    uTempo: { value: 0 }, uOnda: { value: 1 }, uIntensidade: { value: 1 }, uQuadro: { value: 0 },
  ...
  const anel = new Mesh(new TorusGeometry(R, r, celular ? 64 : 96, celular ? 200 : 320), material);
  ...
  return {
    ...
    pintar({ q: e, tempo, entrada, parado }) {
      grupo.position.set(e.x, e.y + (1 - entrada) * -.6, e.z);
      grupo.rotation.set(e.rx * grau, e.ry * grau, e.rz * grau);
      grupo.scale.setScalar(e.s * (.94 + .06 * entrada));
      // Deriva própria, bem lenta: o anel gira no próprio eixo (uma volta a cada ~4 min).
      anel.rotation.z = parado ? 0 : tempo * .026;
      u.uTempo.value = parado ? 0 : tempo;
      u.uOnda.value = e.onda;
      u.uIntensidade.value = e.luz * entrada;
